...
router.post('/queue', auth, async (req, res) => {
  ...
  // --- [디버깅 코드] ---
  console.log(`[DEBUG] POST /queue 요청 받음 | userId: ${userId}, gameTypeId: ${gameTypeId}`);
  ...
  try {
    ...
    // --- [디버깅 코드] ---
    // 사용자를 추가하기 전, 이미 대기열에 있는지 확인하는 로그
    const isAlreadyInQueue = await redisClient.zScore(queueKey, String(userId));
    if (isAlreadyInQueue) {
        console.warn(`[DEBUG] 경고: 사용자 ${userId}는 이미 대기열(${queueKey})에 존재합니다.`);
    }
...
router.delete('/queue', auth, async (req, res) => {
  ...
  // --- [디버깅 코드] ---
  console.log(`[DEBUG] DELETE /queue 요청 받음 | userId: ${userId}, gameTypeId: ${gameTypeId}`);
  ...
  try {
    ...
    const result = await redisClient.zRem(queueKey, String(userId));  // zRem은 제거된 멤버의 수를 반환합니다. (성공 시 1, 대상이 없을 시 0)
